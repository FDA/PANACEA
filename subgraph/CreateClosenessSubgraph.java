 ...
 *
 */
public class CreateClosenessSubgraph extends AbstractCreateNodeSubgraph {	
	Double from, to;
	
	public CreateClosenessSubgraph(Double from, Double to) {
		super(-2);
		this.from = from;
		this.to = to;
	}
	...
	protected GeneralGraph getNetwork() throws IllegalArgumentException, InstantiationException, IllegalAccessException {		
		...
		VAERS_Predicates.ClosenessPredicate pred = new VAERS_Predicates.ClosenessPredicate(super.parent, from, to);
	...
	protected String getName() {
		...
		return ("closeness range ["+df.format(from)+","+df.format(to)+"] subnetwork");
	}
}